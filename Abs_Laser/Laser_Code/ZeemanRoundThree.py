...
# print("AHHH MAGNETIC FIELD",mag_field)
# find_peaks
...
for i in range(29,60):
    ...
    data = pd.read_csv(r"Abs_Laser\Data\24-03-2023\V"+str(i)+"R.CSV")
    data_B = pd.read_csv(r"Abs_Laser\Data\24-03-2023\V"+str(i)+"B.CSV")
    c1 = data['C1 in V']
    c1_B = data_B['C1 in V']
    freq = data['in s']
    ...
    CB_peaks = CB_plot[(freq>freq_min) & (freq<freq_max)]
    freq_peaks = freq[(freq>freq_min) & (freq<freq_max)]
    ...
    if mag_field[2*i] > 0:
        ...
        for point in cb_x_peak:
            ax[1].scatter(mag_field[2*i+1]-7,point,color='blue',marker='x')
            peak_y.append(point)
            peak_x.append(mag_field[2*i+1]-7)


ax[0].axvline(freq_min)
ax[0].axvline(freq_max)
