...
for i in range(0,len(combined_s)):
    ...
    result = spo.basinhopping(func=scatter_difference, niter=50, x0=list([x_guess,y_guess]), T=0, minimizer_kwargs = {"args":(combined_alpha[i],combined_d[i],combined_s[i]),"method":method_,"bounds":([0,20],[0,20])})
...
Clustering UMAP
"""
X_train = dataframe.drop(columns=['x','y'])
...
cluster_0 = drop_preds(X_train,labels,0)
cluster_1 = drop_preds(X_train,labels,1)
