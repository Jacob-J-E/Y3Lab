...
atomic_numbers_k_names = ['Cu', 'Ag', 'Zr', 'Zn','Ti', 'Fe', 'Sn','Mo', 'In', 'Ni']
atomic_numbers_k = [29, 47, 40, 30, 21, 26, 50, 42, 49, 28]

k_alpha = []
k_beta = []

for i,col_name in enumerate(atomic_numbers_k_names):
    ka1 = 0
    kb1 = 0
    for name, line in xraydb.xray_lines(atomic_numbers_k_names[i], 'K').items():
        if name == 'Ka1':
            ka1 = line.energy
        elif name == 'Kb1':
            kb1 = line.energy
    ...
    k_alpha.append(mu_guess_e1)
    k_beta.append(mu_guess_e2)
...
plt.ylabel(r"$\sqrt{\frac{1}{wavelength}}}$ $(m^{-1/2})$")
# plt.grid()
plt.legend()
plt.show()
