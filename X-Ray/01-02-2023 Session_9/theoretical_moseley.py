...
print(f"Alpha Percentage Difference: {100*(R_0 - (4/3)*alpha_fit[0]**2)/R_0}")
print(f"Beta Percentage Difference: {100*(R_0 - (9/8)*beta_fit[0]**2)/R_0}")
...
if plot == True:
    ...
    grad_alpha_40_guess = (4/3)*np.sqrt(R_0)
    ...
    grad_beta_40_guess = (8/9)*np.sqrt(R_0)
    ...
    ax[0].plot(Z,straight_line(Z,*alpha_fit), label = r'Fitted $k_/alpha$ ',color="Black")
    ax[0].plot(Z,straight_line(Z,*beta_fit), label = r'Fitted $k_/beta$',color="Black")
